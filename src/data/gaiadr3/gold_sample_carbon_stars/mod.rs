// This code is generated by generate_code.py, do not modify it manually.

//! This module contains all the known columns in the gold_sample_carbon_stars table.

use crate::traits::{Column, Table};

/// List of \textit{Gaia} DR3 source IDs for carbon stars extracted from the main list of candidate carbon stars for having C2 and CN molecular bands significantly stronger than usual M stars \citep{DR3-DPACP-123}.
#[allow(non_camel_case_types)]
pub struct gold_sample_carbon_stars;

impl Table for gold_sample_carbon_stars {
    fn string(&self) -> String {
        "gold_sample_carbon_stars".to_string()
    }
}

/// The columns in the gold_sample_carbon_stars table.
#[allow(non_camel_case_types)]
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, strum::Display)]
pub enum Col {
    /// Unique source identifier (unique within a particular Data Release)
    source_id,
}

impl Column for Col {}

#[cfg(test)]
/// Collects all the known columns in the gold_sample_carbon_stars table.
pub fn collect_known(map: &mut std::collections::HashMap<String, Vec<String>>) {
    let mut col_strings = Vec::new();
    col_strings.push(Col::source_id.to_string());
    map.insert(gold_sample_carbon_stars.string(), col_strings);
}
